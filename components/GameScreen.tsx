...
export const GameScreen: React.FC<GameScreenProps> = ({ imageUrl, loadingStatus, location, hoveredItem, availableExits }) => {
  ...
  return (
    <div className="relative w-full aspect-video bg-black border-2 border-gray-700 overflow-hidden group shadow-lg">
      ...
      {safeExits.length > 0 && !isBusy && (
         <div className="absolute top-2 right-2 z-30 flex flex-col items-end gap-1">
            ...
            {safeExits.map((exit, i) => (
                <div key={i} className="bg-black/80 px-2 py-0.5 text-green-300 border border-green-800 text-lg hover:bg-green-900 transition-colors cursor-default">
                    {exit}
                </div>
            ))}
         </div>
      ...
      {/* Main Image Layering System */}
      {hasImage ? (
        <>
            {/* Layer 1: Blurred Background (Fills the gap of aspect ratio mismatch) */}
            <img 
                src={imageUrl} 
                alt="Background Ambiance" 
                className="absolute inset-0 w-full h-full object-cover blur-xl opacity-50 scale-110 grayscale-[30%]"
            />

            {/* Layer 2: The Actual Image (Contained to prevent cropping) */}
            <img 
                src={imageUrl} 
                alt="Scene" 
                className="relative z-10 w-full h-full object-contain transition-opacity duration-500 shadow-[0_0_50px_rgba(0,0,0,0.8)]"
      ...
      {/* Loading Overlay */}
      {isBusy && (
        <div className={`absolute inset-0 z-50 flex flex-col items-center justify-center transition-all duration-300 ${hasImage ? 'bg-black/60 backdrop-blur-[2px]' : 'bg-black'}`}>
